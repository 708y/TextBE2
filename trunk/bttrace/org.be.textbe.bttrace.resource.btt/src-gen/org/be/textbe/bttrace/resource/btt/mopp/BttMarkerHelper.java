 ...
 * contains <code>org.be.textbe.bttrace.resource.btt.IBttTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class BttMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = org.be.textbe.bttrace.resource.btt.mopp.BttPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<org.be.textbe.bttrace.resource.btt.IBttCommand<Object>> commands = new java.util.ArrayList<org.be.textbe.bttrace.resource.btt.IBttCommand<Object>>();
		private final Object jobLock = new Object();
		
		public void addCommand(org.be.textbe.bttrace.resource.btt.IBttCommand<Object> command) {
			...
			new org.eclipse.core.runtime.jobs.Job("updating markers") {
				...
				protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
					synchronized(jobLock) {
						java.util.List<org.be.textbe.bttrace.resource.btt.IBttCommand<Object>> commandsToProcess = new java.util.ArrayList<org.be.textbe.bttrace.resource.btt.IBttCommand<Object>>();
						synchronized(commands) {
							commandsToProcess.addAll(commands);
							commands.clear();
						}
						for (org.be.textbe.bttrace.resource.btt.IBttCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final org.be.textbe.bttrace.resource.btt.IBttTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final org.be.textbe.bttrace.resource.btt.IBttTextDiagnostic diagnostic) {
		final org.be.textbe.bttrace.resource.btt.IBttProblem problem = diagnostic.getProblem();
		org.be.textbe.bttrace.resource.btt.BttEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new org.be.textbe.bttrace.resource.btt.IBttCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == org.be.textbe.bttrace.resource.btt.BttEProblemSeverity.ERROR) {
					...
					org.be.textbe.bttrace.resource.btt.IBttTextDiagnostic textDiagnostic = (org.be.textbe.bttrace.resource.btt.IBttTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof org.be.textbe.bttrace.resource.btt.mopp.BttResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((org.be.textbe.bttrace.resource.btt.mopp.BttResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<org.be.textbe.bttrace.resource.btt.IBttQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, org.be.textbe.bttrace.resource.btt.util.BttStringUtil.explode(sourceIDs, "|"));
					}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (org.be.textbe.bttrace.resource.btt.BttEProblemType nextType : org.be.textbe.bttrace.resource.btt.BttEProblemType.values()) {
	 ...
	 */
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, org.be.textbe.bttrace.resource.btt.BttEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(org.be.textbe.bttrace.resource.btt.BttEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	private static String getMarkerID(org.be.textbe.bttrace.resource.btt.BttEProblemType problemType) {
	...
	private static void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			org.be.textbe.bttrace.resource.btt.mopp.BttPlugin.logError("Error while removing markers from resource:", ce);
		}
	}
}
