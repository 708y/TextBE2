 ...
 */
public class BttDebuggableInterpreter<ResultType, ContextType> extends org.be.textbe.bttrace.resource.btt.debug.AbstractBttDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.be.textbe.bttrace.resource.btt.util.AbstractBttInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.be.textbe.bttrace.resource.btt.IBttCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public BttDebuggableInterpreter(org.be.textbe.bttrace.resource.btt.util.AbstractBttInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.be.textbe.bttrace.resource.btt.IBttInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.be.textbe.bttrace.resource.btt.util.BttStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.be.textbe.bttrace.resource.btt.util.AbstractBttInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.be.textbe.bttrace.resource.btt.IBttLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.be.textbe.bttrace.resource.btt.IBttLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
		org.eclipse.emf.ecore.resource.Resource resource = element.eResource();
		if (resource instanceof org.be.textbe.bttrace.resource.btt.IBttTextResource) {
			org.be.textbe.bttrace.resource.btt.IBttTextResource textResource = (org.be.textbe.bttrace.resource.btt.IBttTextResource) resource;
			org.be.textbe.bttrace.resource.btt.IBttLocationMap locationMap = textResource.getLocationMap();
			return locationMap;
		}
		return null;
	}
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.be.textbe.bttrace.resource.btt.IBttCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.be.textbe.bttrace.resource.btt.util.BttEObjectUtil.getDepth(current);
		stopCondition = new org.be.textbe.bttrace.resource.btt.IBttCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.be.textbe.bttrace.resource.btt.util.BttEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.be.textbe.bttrace.resource.btt.util.BttEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.be.textbe.bttrace.resource.btt.IBttCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
