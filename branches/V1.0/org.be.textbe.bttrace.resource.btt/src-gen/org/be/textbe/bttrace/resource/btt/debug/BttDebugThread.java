...
public class BttDebugThread extends org.be.textbe.bttrace.resource.btt.debug.BttDebugElement implements org.eclipse.debug.core.model.IThread, org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener {
	
	private boolean suspended = false;
	private org.be.textbe.bttrace.resource.btt.debug.BttDebugTarget debugTarget;
	
	public BttDebugThread(org.be.textbe.bttrace.resource.btt.debug.BttDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.be.textbe.bttrace.resource.btt.util.BttStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.be.textbe.bttrace.resource.btt.debug.BttStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.be.textbe.bttrace.resource.btt.debug.BttDebugTarget getTarget() {
		return debugTarget;
	}
