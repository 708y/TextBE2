...
public class BttDebugTarget extends org.be.textbe.bttrace.resource.btt.debug.BttDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.be.textbe.bttrace.resource.btt.debug.BttDebugProcess process;
	...
	private org.be.textbe.bttrace.resource.btt.debug.BttDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener> eventListeners = new java.util.ArrayList<org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener>();
	
	private org.be.textbe.bttrace.resource.btt.debug.BttDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.be.textbe.bttrace.resource.btt.mopp.BttMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public BttDebugTarget(org.be.textbe.bttrace.resource.btt.debug.BttDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.be.textbe.bttrace.resource.btt.debug.BttDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.be.textbe.bttrace.resource.btt.debug.BttDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.be.textbe.bttrace.resource.btt.debug.IBttDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.be.textbe.bttrace.resource.btt.mopp.BttMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.be.textbe.bttrace.resource.btt.debug.BttLineBreakpoint lineBreakpoint = (org.be.textbe.bttrace.resource.btt.debug.BttLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message) {
		try {
			if (message.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.RESUMED)) {
	...
	public org.be.textbe.bttrace.resource.btt.debug.BttDebugProxy getDebugProxy() {
