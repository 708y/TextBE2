 ...
 * code).
 */
public class BttDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private org.be.textbe.bttrace.resource.btt.debug.AbstractBttDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, org.be.textbe.bttrace.resource.btt.util.BttPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, org.be.textbe.bttrace.resource.btt.util.BttPair<String, Object>>();
	...
	public BttDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = org.be.textbe.bttrace.resource.btt.util.BttStringUtil.encode('#', stack);
				org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message = new org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message = new org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_VARIABLES)) {
				...
				org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage message = new org.be.textbe.bttrace.resource.btt.debug.BttDebugMessage(org.be.textbe.bttrace.resource.btt.debug.EBttDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return org.be.textbe.bttrace.resource.btt.util.BttStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		org.be.textbe.bttrace.resource.btt.util.BttPair<String, Object> pair = new org.be.textbe.bttrace.resource.btt.util.BttPair<String, Object>(name, value);
	...
	public org.be.textbe.bttrace.resource.btt.debug.AbstractBttDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(org.be.textbe.bttrace.resource.btt.debug.AbstractBttDebuggable debuggable) {
